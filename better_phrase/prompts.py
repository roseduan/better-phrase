...
POLISH_INSTRUCTIONS = """\
The user's input contains English. Before answering their actual request, prepend an English polish block in this EXACT format:

> ✏️ **English tip:**
> - "original phrase" → "corrected phrase" — one-line grammar rule explanation (in Chinese)
> - (up to 4-5 of the most instructive issues)
>
> ✍️ **Better version:** rewrite the user's ENTIRE original English input into natural, idiomatic English — the kind a native speaker would write.
...
- The "Better version" must rewrite the WHOLE input for natural flow, not just patch errors.
...
>
> ✍️ **Better version:** "The background color of this section is black, which clashes with the overall site style. It looks fine in dark mode, but doesn't work in light mode."
"""


TRANSLATION_INSTRUCTIONS = """\
The user's input is primarily Chinese. Before answering their actual request, prepend a Chinese-to-English version block in this EXACT format:
...
Immediately after the BP block above (the ✏️ tip / 🌐 translation quoted section, plus any hint footer), and BEFORE you start answering the user's actual request, append a single timing line on its own, in italics:

*(⏱ Better Phrase: __HOOK_MS__ms hook)*

Placement is strict: the timing line belongs with the BP block, NOT at the end of your response. Order is: BP block → (hint footer if present) → timing line → blank line → your normal answer.

Format the line cleanly, e.g.: *(⏱ Better Phrase: 52ms hook)*
"""
